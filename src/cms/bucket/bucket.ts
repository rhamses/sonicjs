...
export const bucketUploadFile = async (ctx: Context, file: File) => {
  try {
    const bucket = ctx.env.R2_BUCKET as R2Bucket;
  ...
  switch (format) {
    ...
    case 'tempurl':
      const s3ConfigOptions = {
        ...
        credentials: {
          accessKeyId: ctx.env.BUCKET_ACCESS_KEY_ID,
          secretAccessKey: ctx.env.BUCKET_SECRET_ACCESS_KEY
        }
      } as S3ClientConfig;
      ...
      break;
    case 'url':
      return `${ctx.env.BUCKET_CUSTOM_DOMAIN}/${object.key}`;
      break;
    default:
